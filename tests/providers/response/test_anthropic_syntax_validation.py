...
- claude-3-5-haiku-20241022 (for all normal operations)
- claude-sonnet-4-20250514 (for thinking mode testing)
"""
...
class TestAnthropicSyntaxValidation:
    ...
    def response_dir(self) -> Path:
        ...
        (response_dir / "claude-sonnet-4").mkdir(exist_ok=True)
        return response_dir

    def get_test_cases(self) -> List[Dict[str, Any]]:
        """Get all test cases for syntax validation."""
        return [
            # Test 1: haiku basic call
            {
            ...
            # Test 4: haiku with stop sequences
            {
                "test_id": "test_4_haiku_stop_sequences",
                "model": "claude-3-5-haiku-20241022",
                "params": {
            ...
            # Test 5: haiku single tool call
            {
                "test_id": "test_5_haiku_single_tool",
                "model": "claude-3-5-haiku-20241022",
                "params": {
                    "messages": [
            ...
            # Test 6: haiku multiple tools
            {
                "test_id": "test_6_haiku_multiple_tools",
                "model": "claude-3-5-haiku-20241022",
                "params": {
                    "messages": [
                        {
            ...
            # Test 7: haiku multi-turn conversation
            {
                "test_id": "test_7_haiku_multi_turn",
                "model": "claude-3-5-haiku-20241022",
                "params": {
                    "messages": [
            ...
            # Test 8: haiku streaming response
            {
                "test_id": "test_8_haiku_streaming",
                "model": "claude-3-5-haiku-20241022",
                "params": {
            ...
            # Test 9: Error case - invalid parameter
            {
            ...
            # Test 10: haiku long response
            {
                "test_id": "test_10_haiku_long_response",
                "model": "claude-3-5-haiku-20241022",
                "params": {
                    "messages": [
                        {
            ...
            # Test 11: haiku forced tool calling
            {
                "test_id": "test_11_haiku_forced_tool",
                "model": "claude-3-5-haiku-20241022",
                "params": {
            ...
            # Test 12: claude-4-sonnet with interleaved thinking and tools
            {
                "test_id": "test_12_sonnet_thinking_tools",
                "model": "claude-sonnet-4-20250514",
                "params": {
                    "messages": [
                        {"role": "user", "content": "Calculate 150 units at $50 each"}
                    ],
                ...
                "expected_content_types": ["thinking", "tool_use"],
                "extra_headers": {"anthropic-beta": "interleaved-thinking-2025-05-14"},
            },
        ]
    ...
    async def execute_test_case(
        ...
        if "haiku" in model:
            ...
            model_dir = "claude-sonnet-4"

        storage_path = response_dir / model_dir / f"{test_id}.json"

        try:
            # Add extra headers if specified
            if "extra_headers" in test_case:
                client._client.headers.update(test_case["extra_headers"])
    ...
    def validate_response_fields(
        ...
        if response.get("streaming", False):
            ...
            if not chunks:
                ...
                message_start = next(
                    (c for c in chunks if c.get("type") == "message_start"), None
                )
                if message_start:
                ...
                message_stop = next(
                    (c for c in chunks if c.get("type") == "message_stop"), None
                )
                if (
                    message_stop
                    and "usage" in test_case.get("expected_fields", [])
                    and "usage" not in message_stop.get("message", {})
                ):
                ...
                if "expected_content_types" in test_case:
                    ...
                    for expected_type in test_case["expected_content_types"]:
                        if expected_type not in content_types:
                            errors.append(
                                f"Missing expected content type: {expected_type}"
                            )
                ...
                for block in content:
                    if block.get("type") == "tool_use":
                        ...
                        if "thinking" not in block:
                            errors.append("Missing thinking field in thinking block")

        # Validate stop reason
        if (
            "expected_stop_reason" in test_case
            and response_data.get("stop_reason") != test_case["expected_stop_reason"]
        ):
            errors.append(
    ...
    async def test_syntax_validation(self, response_dir: Path):
        ...
        summary_path = response_dir / "anthropic_validation_summary_v2.json"
    ...
    def test_stored_responses_exist(self, response_dir: Path):
        ...
        haiku_dir = response_dir / "claude-3-5-haiku"
        sonnet4_dir = response_dir / "claude-sonnet-4"
        
        # Check if directories exist
        assert haiku_dir.exists(), "claude-3-5-haiku directory not found"
        assert sonnet4_dir.exists(), "claude-sonnet-4 directory not found"
        
        # Check for responses (may be empty if tests haven't been run with API key)
        haiku_responses = list(haiku_dir.glob("*.json"))
        sonnet4_responses = list(sonnet4_dir.glob("*.json"))
        
        if len(haiku_responses) == 0 and len(sonnet4_responses) == 0:
            pytest.skip("No stored responses found - run with ANTHROPIC_API_KEY to generate")
        
        # If we have responses, verify summary exists (either v1 or v2)
        if len(haiku_responses) > 0 or len(sonnet4_responses) > 0:
            summary_v1 = response_dir / "anthropic_validation_summary.json"
            summary_v2 = response_dir / "anthropic_validation_summary_v2.json"
            assert summary_v1.exists() or summary_v2.exists(), "No validation summary found"
    ...
    def test_response_validator_completeness(self):
        ...
        for method_name in required_methods:
            assert hasattr(AnthropicResponseValidator, method_name), (
