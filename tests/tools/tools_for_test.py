...
def get_station_list() -> List[Dict[str, str]]:
    ...
    """
    return [
        {"city": city, "name": info["name"]} for city, info in WEATHER_STATIONS.items()
    ]
...
def get_weather(city: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            ...
            or "data" not in data["observations"]
            or not data["observations"]["data"]
        ):
        ...
        local_time = datetime.strptime(
            latest_data["local_date_time_full"], "%Y%m%d%H%M%S"
        )
        ...
        error_msg = f"Failed to parse weather data: {e!s}"
        logger.error(error_msg)
        return {"error": error_msg}
    except Exception as e:
        error_msg = f"Error getting weather data: {e!s}"
        logger.error(error_msg)
        return {"error": error_msg}
    finally:
        conn.close()
