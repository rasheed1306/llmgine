...
import pytest
from tools_for_test import WEATHER_STATIONS, get_station_list, get_weather
...
def validate_weather_data(data: Dict[str, Any], expected_city: str) -> None:
    ...
    assert all(field in data for field in required_fields), (
        f"Missing required fields in data: {data}"
    )
...
def test_get_weather_for_all_cities():
    ...
    for city in WEATHER_STATIONS.keys():
        ...
        assert "error" not in data, (
            f"Error getting weather for {city}: {data.get('error')}"
        )
