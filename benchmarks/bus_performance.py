...
class BenchmarkRunner:
    ...
    async def benchmark_sustained_throughput(
        self, target_ops: int = 100000, target_rate: int = 10000
    ) -> BenchmarkResult:
        ...
        rate_limiter = asyncio.Semaphore(
            target_rate // 10
        )  # Allow bursts of 1/10th the target rate
            ...
            for i in range(target_ops):
                async with rate_limiter:
                    try:
                        ...
                        await bus.publish(
                            BenchmarkEvent(payload={"index": i, "data": "x" * 100})
                        )
                        publish_time = (
                    ...
                    asyncio.create_task(
                        self._release_after_delay(rate_limiter, 1.0 / target_rate)
                    )
    ...
    async def benchmark_latency_under_load(
        self, ops_per_batch: int = 1000, num_batches: int = 10
    ) -> BenchmarkResult:
        ...
        for batch in range(num_batches):
            ...
            for i in range(concurrent_ops):
                publish_start = time.time()
                task = bus.publish(BenchmarkEvent(payload={"batch": batch, "index": i}))
        ...
        total_expected = sum(
            ops_per_batch
            if i % 3 == 0
            else ops_per_batch // 2
            if i % 3 == 1
            else ops_per_batch // 10
    ...
    async def benchmark_memory_stability(
        self, duration_seconds: int = 30, rate: int = 5000
    ) -> BenchmarkResult:
        ...
        while time.time() < end_time:
            ...
            for _ in range(batch_size):
                try:
                    publish_start = time.time()
                    await wrapped_bus.publish(
                        BenchmarkEvent(payload={"data": "x" * random.randint(50, 200)})
                    )
    ...
    async def benchmark_chaos_testing(
        self, duration_seconds: int = 20, failure_rate: float = 0.1
    ) -> BenchmarkResult:
        ...
        while time.time() < end_time:
            ...
            try:
                await bus.publish(
                    BenchmarkEvent(payload={"chaos": True, "index": operations})
                )
                latencies.append((time.time() - publish_start) * 1000)
                operations += 1
            except Exception:
    ...
    def print_results(self):
        ...
        for result in self.results:
            ...
            print(
                f"Latency (ms): p50={result.latency_p50_ms:.2f}, p95={result.latency_p95_ms:.2f}, p99={result.latency_p99_ms:.2f}"
            )
            print(
                f"Memory: Start={result.memory_start_mb:.1f}MB, Peak={result.memory_peak_mb:.1f}MB, End={result.memory_end_mb:.1f}MB"
            )
        ...
        sustained_result = next(
            (r for r in self.results if r.test_name == "Sustained Throughput"), None
        )
        if sustained_result:
            target_met = sustained_result.throughput_ops_per_sec >= 10000
            status = "✅ PASS" if target_met else "❌ FAIL"
            print(
                f"Throughput Target (10k ops/sec): {status} - Achieved {sustained_result.throughput_ops_per_sec:,.0f} ops/sec"
            )

            latency_met = sustained_result.latency_p99_ms < 10
            status = "✅ PASS" if latency_met else "❌ FAIL"
            print(
                f"Latency Target (p99 < 10ms): {status} - Achieved {sustained_result.latency_p99_ms:.2f}ms"
            )
        ...
        for result in self.results:
            if result.throughput_ops_per_sec < 10000:
                recommendations.append(
                    f"- {result.test_name}: Consider optimizing handler performance or increasing concurrency"
                )
            if result.latency_p99_ms > 10:
                recommendations.append(
                    f"- {result.test_name}: High p99 latency detected, investigate blocking operations"
                )
            if result.memory_peak_mb - result.memory_start_mb > 100:
                recommendations.append(
                    f"- {result.test_name}: Significant memory growth detected, check for memory leaks"
                )
            if result.cpu_percent_avg > 80:
                recommendations.append(
                    f"- {result.test_name}: High CPU usage, consider optimizing hot paths"
                )

        if not recommendations:
            recommendations.append(
                "- All performance targets met! Consider testing with higher loads."
            )
...
async def main():
    ...
    try:
        ...
        result = await runner.benchmark_latency_under_load(
            ops_per_batch=1000, num_batches=10
        )
        print(f"   ✓ Completed: p99 latency {result.latency_p99_ms:.2f}ms")

        # 3. Memory stability test
        result = await runner.benchmark_memory_stability(duration_seconds=10, rate=5000)
        print(
            f"   ✓ Completed: Memory growth {result.memory_peak_mb - result.memory_start_mb:.1f}MB"
        )

        # 4. Chaos testing
        result = await runner.benchmark_chaos_testing(
            duration_seconds=10, failure_rate=0.2
        )
